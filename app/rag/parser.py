...
def _load_tree_sitter_parser(ext: str):
    ...
    try:
        ...
        if ext in {".js", ".jsx"}:
            ...
            language = ts.Language(ts_ts.language_typescript())
        elif ext == ".tsx":
            import tree_sitter_typescript as ts_ts
            language = ts.Language(ts_ts.language_tsx())
        else:
            return None
...
def _build_file_chunks(file_path: str, source: str, use_tree_sitter: bool = True) -> tuple[list[dict[str, Any]], list[str], list[str], list[str]]:
...
def parse_repo(repo_path: str, extensions=None, use_tree_sitter: bool = True):
